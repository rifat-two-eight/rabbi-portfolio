...
export default function Tools() {
    ...
    return (
        <section id="tools" className="scroll-mt-32 px-6 max-w-7xl mx-auto pb-24 space-y-12">
            {/* Section Header */}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {tools.map((tool, index) => (
                    <FadeIn delay={0.2 + index * 0.1} direction="up" key={tool.name}>
                        <div 
                            className="bg-[#111111]/40 backdrop-blur-xl p-5 rounded-2xl border border-white/30 flex items-center gap-4 hover:border-[#FF6B00]/40 transition-all group cursor-default h-full"
                        >
                            <div className="w-12 h-12 flex items-center justify-center shrink-0">
                                {tool.isPs ? (
                                    <div className="w-10 h-10 bg-[#001E36] border-2 border-[#31A8FF] rounded-lg flex items-center justify-center">
                                        <span className="text-[#31A8FF] font-semibold text-xl">Ps</span>
                                    </div>
                                ) : (
                                    <Image 
